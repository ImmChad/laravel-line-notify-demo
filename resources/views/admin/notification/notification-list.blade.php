            ...
            <div class="card-header" style="padding: 10px 40px;">
                <div class="media" style="display: flex; padding: 0px;">
                    <form class="form-inline"
                          style=" border: 1px solid #f4f4f4;  padding: 0px 20px; border-radius: 5px;">
                        <div class="form-group mb-0" style="justify-content: center; align-items: center;">
                            ...
                            <input id="ipt-search-notification" class="form-control-plaintext" type="text"
                                   value="{{isset($_GET['txt-search-notification'])?$_GET['txt-search-notification']:''}}"
                                   placeholder="Search title notification">
                            <select id="ipt-select-filter-template" class="form-select"
                    ...
                    @foreach ($dataList as $subDataList )
                        @if($subDataList->deleted_at == null)
                            <tr class="link-detail-announce" data-id-notification='{{$subDataList->id}}' style="">
                                ...
                                <td style="white-space: nowrap">{!! strlen($subDataList->announce_title)>40?substr($subDataList->announce_title,0,40)."...":$subDataList->announce_title !!}</td>
                                <td>{{$subDataList->created_at}}</td>
                                <td>{{$subDataList->read_user}}/{{$subDataList->total_user}}</td>
                                <td>
                                    <button class="btn btn-success btn-update-notification"
                                            data-id-notification='{{$subDataList->id}}' {{$subDataList->is_sent==1?'disabled':''}} >
                                        Update
                                    </button>
                                    <button class="btn btn-danger btn-delete-notification"
                                            data-id-notification='{{$subDataList->id}}'>Delete
                                    </button>
                                    <button class="btn btn-primary btn-show-details-notification"
                                            data-id-notification='{{$subDataList->id}}'>Show
                                    </button>
                                </td>
                            </tr>
                        @else
                            <tr class="link-detail-announce" data-id-notification='{{$subDataList->id}}'>
                                ...
                                <td style="color: #b6afaf !important; white-space: nowrap">{!! strlen($subDataList->announce_title)>40?substr($subDataList->announce_title,0,40)."...":$subDataList->announce_title !!}</td>
                                <td style="color: #b6afaf !important;">{{$subDataList->created_at}}</td>
                                <td style="color: #b6afaf !important;">{{$subDataList->read_user}}
                                    /{{$subDataList->total_user}}</td>
                                <td>
                                    <button class="btn btn-secondary btn-update-notification" disabled
                                            style="background-color: #6c757d !important; border-color: #6c757d  !important;">
                                        Update
                                    </button>
                                    <button class="btn btn-secondary btn-delete-notification" disabled
                                            style="background-color: #6c757d !important; border-color: #6c757d  !important;">
                                        Delete
                                    </button>
                                    <button class="btn btn-secondary btn-show-details-notification"
                                            data-id-notification='{{$subDataList->id}}' disabled
                                            style="background-color: #6c757d !important; border-color: #6c757d  !important;">
                                        Show
                                    </button>
                                </td>
