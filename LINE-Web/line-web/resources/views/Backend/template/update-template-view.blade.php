                            ...
                            <div class="media">
                                <div class="media-body">
                                    <div class="mb-3">
                                        ...
                                        <input value="{{$dataTemplate->template_name}}" class="form-control" id="template_name" type="text" placeholder="Enter title ...">
                                    </div>
                                </div>
                            </div>
                                ...
                                <li style="padding: 1rem 0rem; width: 100%;">
                                    <select class="form-select digits" id="dataRegion">
                                        ...
                                        @foreach ($dataRegion as $subDataRegion)
                                            @if ($dataTemplate->region_id == $subDataRegion->id)
                                                <option region_id="{{ $subDataRegion->id }}" region_name="{{ $subDataRegion->region_name }}" selected>{{ $subDataRegion->region_name }}</option>
                                ...
                                <li style="padding: 1rem 0rem; width: 100%;">
                                    <select class="form-select digits" id="dataArea">
                                        ...
                                        @foreach ($dataArea as $subDataArea)
                                            @if ($dataTemplate->area_id == $subDataArea->id)
                                                <option area_id="{{ $subDataArea->id }}" area_name="{{ $subDataArea->area_name }}" selected>{{ $subDataArea->area_name }}</option>
                                            @else
                                ...
                                <li style="padding: 1rem 0rem; width: 100%;">
                                    <select class="form-select digits" id="dataIndustry">
                                        ...
                                        @foreach ($dataIndustry as $subDataIndustry)
                                            @if ($dataTemplate->industry_id == $subDataIndustry->id)
                                                <option industry_id="{{ $subDataIndustry->id }}" industry_name="{{ $subDataIndustry->industry_name }}" selected>{{ $subDataIndustry->industry_name }}</option>
                                            @else
                                ...
                                <li style="padding: 1rem 0rem; width: 100%;">
                                    <select class="form-select digits" id="dataStore">
                                        ...
                                        @foreach ($dataStore as $subDataStore)
                                            @if ($dataTemplate->store_id == $subDataStore->id)
                                                <option store_id="{{ $subDataStore->id }}" store_name="{{ $subDataStore->store_name }}" selected>{{ $subDataStore->store_name }}</option>                                            
                                            @else
                ...
                <div class="contact-form card-body">
                    <form class="theme-form">
                        ...
                        <div class="mb-3">
                            ...
                            <input class="form-control" id="announce_title" type="text" placeholder="Enter title ..." value="{{ $dataTemplate->template_title }}">
                        </div>
                        <div class="mb-3">
                        ...
                        <div class="text-sm-end">
                            <button data-template-id="{{$dataTemplate->id}}" id="btn-submit-update-template" class="btn btn-primary-gradien btn-send-it">UPDATE TEMPLATE</button>
                        </div>
                    </form>
                </div>
            </div>
...
@section('script')
    ...
    <script >
        @if (isset($dataTemplate->template_content))
            window.addEventListener('load',()=>{
                var announce_content = CKEDITOR.instances['announce_content'];
                console.log(`{!! $dataTemplate->template_content !!}`);
                let newDiv = `{!! $dataTemplate->template_content !!}`;
        ...
        dataArea.addEventListener('change', (e) => {
            ...
            if(area_id != 0) {
                if(dataArea != null ) {
                    ...
                    let newDiv = '<div id="dataArea">[' + area_name + ']</div><p>&nbsp;</p>';
        ...
        var btn_update_template = document.querySelector("#btn-submit-update-template");

        btn_update_template.addEventListener('click',(event)=>{
            ...
            var id = btn_update_template.getAttribute('data-template-id');
            reqUpdateTemplate(
            id,
            template_name,
            template_title,
            ...
            if(template_name.length>0 && template_title.length>0 && template_content.length>0)
            {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if(data.result)
                        {
                            displayToast("Successfully Update new template ")
